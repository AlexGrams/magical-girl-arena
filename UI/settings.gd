...
func _ready() -> void:
	...
	_volume_slider.value = settings.get_value("sound", "volume") / max_volume_slider_value
	_spinbox.value = _volume_slider.value * 100
...
func _save_settings_changes() -> void:
	SaveManager.save_settings(_screen_modes[_screen_mode_option.selected], _volume_slider.value * max_volume_slider_value)
